...
If two threads might edit the same files, you can isolate one in a Git worktree created with `git worktree add` and opened as a separate project. See [Worktree Isolation](./parallel-agents.md#worktree-isolation) for details.
